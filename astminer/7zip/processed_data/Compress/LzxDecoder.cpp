...
bool CDecoder::ReadTable(Byte *levels, unsigned numSymbols) { {
    ...
    { if (!(_levelDecoder.Build(levels2))) return false; }; }
  unsigned i = 0;
  do {
...
bool CDecoder::ReadTables(void) { {
    ...
    if (blockType == kBlockType_Aligned) {
      ...
      { if (!(_alignDecoder.Build(levels))) return false; }; } }
  { if (!(ReadTable(_mainLevels, 256))) return false; };
  { if (!(ReadTable(_mainLevels + 256, _numPosLenSlots))) return false; };
  unsigned end = 256 + _numPosLenSlots;
  memset(_mainLevels + end, 0, kMainTableSize - end);
  { if (!(_mainDecoder.Build(_mainLevels))) return false; };
  { if (!(ReadTable(_lenLevels, kNumLenSymbols))) return false; };
