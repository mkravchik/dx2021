...
namespace NCompress {
namespace NLzms {
static UInt32 g_PosBases[k_NumPosSyms ];
  ...
  _bs.Init(in, inSize); { { {
    ...
    while (_pos < outSize) {
      if (_rc.Decode(&mainState, k_NumMainProbs, mainProbs) == 0) {
        UInt32 number;
        number = m_LitDecoder.DecodeFull(&_bs); m_LitDecoder.Freqs[number]++; if (--m_LitDecoder.RebuildRem == 0) m_LitDecoder.Rebuild();;
        ...
        if (_rc.Decode(&lzRepStates[0], k_NumRepProbs, lzRepProbs[0]) == 0) {
          UInt32 number;
          number = m_PosDecoder.DecodeFull(&_bs); m_PosDecoder.Freqs[number]++; if (--m_PosDecoder.RebuildRem == 0) m_PosDecoder.Rebuild();;
          if (_bs._buf < _rc.cur) return S_FALSE;
          unsigned numDirectBits = g_PosDirectBits[number];
          distance = g_PosBases[number];
          if (_bs._buf < _rc.cur) return S_FALSE; if ((size_t)(_bs._buf - _rc.cur) < (numDirectBits >> 3)) return S_FALSE;;
          ...
          if (_rc.Decode(&lzRepStates[1], k_NumRepProbs, lzRepProbs[1]) == 0) {
            ...
            if (prevType != 1) {
              ...
              _reps[0] = distance; } } }
        UInt32 lenSlot;
        lenSlot = m_LenDecoder.DecodeFull(&_bs); m_LenDecoder.Freqs[lenSlot]++; if (--m_LenDecoder.RebuildRem == 0) m_LenDecoder.Rebuild();;
        if (_bs._buf < _rc.cur) return S_FALSE;
        ...
        if (_rc.Decode(&deltaRepStates[0], k_NumRepProbs, deltaRepProbs[0]) == 0) {
          power = m_PowerDecoder.DecodeFull(&_bs); m_PowerDecoder.Freqs[power]++; if (--m_PowerDecoder.RebuildRem == 0) m_PowerDecoder.Rebuild();;
          if (_bs._buf < _rc.cur) return S_FALSE;
          UInt32 number;
          number = m_DeltaDecoder.DecodeFull(&_bs); m_DeltaDecoder.Freqs[number]++; if (--m_DeltaDecoder.RebuildRem == 0) m_DeltaDecoder.Rebuild();;
          if (_bs._buf < _rc.cur) return S_FALSE;
          unsigned numDirectBits = g_PosDirectBits[number];
