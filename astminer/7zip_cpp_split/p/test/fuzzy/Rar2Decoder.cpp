...
bool CDecoder::ReadTables(void) {
  ...
  { if (!(m_LevelDecoder.Build(levelLevels))) return false; };
  i = 0;
  do {
  ...
  if (m_AudioMode)
    for (i = 0; i < m_NumChannels; i++) {
      { if (!(m_MMDecoders[i].Build(&lens[i * kMMTableSize]))) return false; }; }
  else {
    { if (!(m_MainDecoder.Build(&lens[0]))) return false; };
    { if (!(m_DistDecoder.Build(&lens[kMainTableSize]))) return false; };
    { if (!(m_LenDecoder.Build(&lens[kMainTableSize + kDistTableSize]))) return false; }; }
...
bool CDecoder::ReadLastTables() {
  ...
  if (m_InBitStream.GetProcessedSize() + 7 <= m_PackSize)
...
HRESULT CDecoder::CodeReal(ISequentialInStream *inStream, ISequentialOutStream *outStream,
  ...
  if (!_isSolid) {
    InitStructures();
    if (unPackSize == 0) {
      if (m_InBitStream.GetProcessedSize() + 2 <= m_PackSize)
        if (!ReadTables())
          return S_FALSE;
      _solidAllowed = true;
      return S_OK; }
