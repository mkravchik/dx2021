...
enum EState {
  ...
  b = -1;
  { if (_numBits < 8) { if (_buf == _lim) return SZ_OK; { _value |= (UInt32)(*_buf) << (24 - _numBits); _numBits += 8; _buf++; \ } } b = _value >> (32 - 8); _value <<= 8; _numBits -= 8; \ };
  return SZ_OK; }
MY_NO_INLINE
...
SRes CBase::ReadBlock2() {
  ...
  if (state == STATE_BLOCK_START) {
    if (Props.randMode) {
      { if (_numBits < 1) { if (_buf == _lim) return SZ_OK; { _value |= (UInt32)(*_buf) << (24 - _numBits); _numBits += 8; _buf++; \ } } Props.randMode = _value >> (32 - 1); _value <<= 1; _numBits -= 1; \ }; }
    state = STATE_ORIG_BITS;

  }
  if (state == STATE_ORIG_BITS) {
    { while (_numBits < kNumOrigBits) { if (_buf == _lim) return SZ_OK; { _value |= (UInt32)(*_buf) << (24 - _numBits); _numBits += 8; _buf++; \ } } Props.origPtr = _value >> (32 - kNumOrigBits); _value <<= kNumOrigBits; _numBits -= kNumOrigBits; \ };
    if (Props.origPtr >= blockSizeMax)
      return SZ_ERROR_DATA;
    state = STATE_IN_USE; }

  if (state == STATE_IN_USE) {
    { while (_numBits < 16) { if (_buf == _lim) return SZ_OK; { _value |= (UInt32)(*_buf) << (24 - _numBits); _numBits += 8; _buf++; \ } } state2 = _value >> (32 - 16); _value <<= 16; _numBits -= 16; \ };
    state = STATE_IN_USE2;
    state3 = 0;
    numInUse = 0;
    mtf.StartInit(); }
  if (state == STATE_IN_USE2) {
    for (; state3 < 256; state3++)
      if (state2 & ((UInt32)0x8000 >> (state3 >> 4))) {
        unsigned b;
        { if (_numBits < 1) { if (_buf == _lim) return SZ_OK; { _value |= (UInt32)(*_buf) << (24 - _numBits); _numBits += 8; _buf++; \ } } b = _value >> (32 - 1); _value <<= 1; _numBits -= 1; \ };
        if (b)
  ...
  if (state == STATE_NUM_TABLES) {
    { if (_numBits < kNumTablesBits) { if (_buf == _lim) return SZ_OK; { _value |= (UInt32)(*_buf) << (24 - _numBits); _numBits += 8; _buf++; \ } } numTables = _value >> (32 - kNumTablesBits); _value <<= kNumTablesBits; _numBits -= kNumTablesBits; \ };
    state = STATE_NUM_SELECTORS;
    if (numTables < kNumTablesMin || numTables > kNumTablesMax)
      return SZ_ERROR_DATA; }
  if (state == STATE_NUM_SELECTORS) {
    { while (_numBits < kNumSelectorsBits) { if (_buf == _lim) return SZ_OK; { _value |= (UInt32)(*_buf) << (24 - _numBits); _numBits += 8; _buf++; \ } } numSelectors = _value >> (32 - kNumSelectorsBits); _value <<= kNumSelectorsBits; _numBits -= kNumSelectorsBits; \ };
  ...
  if (state == STATE_LEVELS) {
    do {
      if (state3 == 0) {
        { if (_numBits < kNumLevelsBits) { if (_buf == _lim) return SZ_OK; { _value |= (UInt32)(*_buf) << (24 - _numBits); _numBits += 8; _buf++; \ } } state3 = _value >> (32 - kNumLevelsBits); _value <<= kNumLevelsBits; _numBits -= kNumLevelsBits; \ };
        state4 = 0;
        state5 = 0; }
  ...
  if (state != STATE_BLOCK_SYMBOLS)
    ...
    UInt32 _value2 = this->_value; UInt32 blockSize2 = this->blockSize; UInt32 runCounter2 = this->runCounter; #define SAVE_LOCAL this->_value = _value2; this->blockSize = blockSize2; this->runCounter = runCounter2; SRes CBitDecoder::ReadByte(int &b) {
    ...
    for (;;) {
      if (groupSize == 0) {
        ...
        groupSize = kGroupSize; }
      if (_numBits <= 8 &&
          _buf != _lim) { { _value2 |= (UInt32)(*_buf) << (24 - _numBits); _numBits += 8; _buf++; \ }
      if (_buf != _lim) { { _value2 |= (UInt32)(*_buf) << (24 - _numBits); _numBits += 8; _buf++; \ }
      if (_buf != _lim) { { _value2 |= (UInt32)(*_buf) << (24 - _numBits); _numBits += 8; _buf++; \ } }}}
      UInt32 sym;
      UInt32 val = _value2 >> (32 - kMaxHuffmanLen);
      if (val >= huff->_limits[kNumTableBits]) {
        if (_numBits <= kMaxHuffmanLen && _buf != _lim) { { _value2 |= (UInt32)(*_buf) << (24 - _numBits); _numBits += 8; _buf++; \ }
        if (_numBits <= kMaxHuffmanLen && _buf != _lim) { { _value2 |= (UInt32)(*_buf) << (24 - _numBits); _numBits += 8; _buf++; \ } }}
        val = _value2 >> (32 - kMaxHuffmanLen);
        unsigned len;
      ...
      groupSize--;
      if (sym < 2) {
        runCounter2 += ((UInt32)(sym + 1) << runPower);
        runPower++;
        if (blockSizeMax - blockSize2 < runCounter2)
          return SZ_ERROR_DATA;
        continue; }
      UInt32 *counters = this->Counters;
      if (runCounter2 != 0) {
        UInt32 b = (UInt32)(mtf.Buf[0] & 0xFF);
        counters[b] += runCounter2;
        runPower = 0;

          Byte *dest = (Byte *)(&counters[256 + kBlockSizeMax]) + blockSize2;
          const Byte *limit = dest + runCounter2;
          blockSize2 += runCounter2;
          runCounter2 = 0;
          do {
            dest[0] = (Byte)b;
            dest[1] = (Byte)b;
      ...
      if (sym < numInUse) {
        ...
        CMtfVar prev = (next >> pos) & 0xFF;
          ...
          ((Byte *)(counters + 256 + kBlockSizeMax))[blockSize2++] = (Byte)prev;
      ...
      ((Byte *)(Counters + 256 + kBlockSizeMax))[blockSize2] = 0;
    ...
    for (;;) {
      ...
      if (
        ...
        if (props.blockSize != 0) {
          ...
          const UInt32 k_Mt_BlockSize_Threshold = (1 << 12);
...
HRESULT CDecoder::CreateThread() {
  { WRes __result_ = (DecoderEvent.CreateIfNotCreated()); if (__result_ != 0) return __result_; };
  { WRes __result_ = (ScoutEvent.CreateIfNotCreated()); if (__result_ != 0) return __result_; };
  { WRes __result_ = (Thread.Create(RunScout2, this)); if (__result_ != 0) return __result_; };
  return S_OK; }
    ...
    try {
      ...
      for (;;) {
        ...
        if (Base.state == STATE_STREAM_FINISHED) {
          ...
          if (res != S_OK)
            break;
          if (GetInputProcessedSize() - packPos > 0)
            break;
          continue; }
